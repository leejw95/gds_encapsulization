        ...
        for field, old_value in ast.iter_fields(node):
            if type(old_value) == str:
                ...
                if old_value[:2] in ['>h', '>H', '^\\', '>Q', '>d']:
                    ...
                    pass
                elif old_value not in ['self', 'print', 'range', 're', 'math', 'datetime', 'MINYEAR', 'copy', 'open', 'readlines', 'readline', 'format', 'now', 'sha256',
                                     'Exception','and','exec','not','assert','finally','or','break','for','pass','class','from','print', 'seek', 'close', 'items',
                                     ...
                                     'pack', 'calcsize', 'write', 'year', 'month', 'day', 'hour', 'minute', 'second', '__name__','__main__', 'items', 'hashlib','/tsmcN65.layermap',
                                       ...
                                       "Connect to Internet", 'urllib', 'request','urlopen','headers','http://www.kriss.re.kr','Date','Jul 2021','Aug 2021', 'Sep 2021', 'Oct 2021',
...
def main_1():
    DP = ast.parse(open('./designs_modified/DesignParameters.py').read())
    DP_1 = name_change()
    ...
    DRC = ast.parse(open('./designs_modified/DRC.py').read())
    DRC_1 = name_change()
    ...
    Stick = ast.parse(open('./designs_modified/StickDiagram.py').read())
    Stick_1 = name_change()
